  ...
  const handleSignup = async (e) => {
    ...
    try {
      const response = await axios.post('/api/signup', {
        ...
        fullName: 'Your full name' // You should add logic to capture the full name from the form
      });
  
      if (response.status === 201) { // Check if the response status is success
        setSuccess('Signup successful!');
        navigate("/login");
        // Optionally, redirect to login page or other actions after signup
      }
    } catch (error) {
