...
const SaleTransactions = () => {
  ...
  const filteredSales = sales?.filter((sale) => {
    ...
    const matchesPrice =
      ...
      (filterPrice === "medium" && sale.price >= 50000 && sale.price <= 100000) ||
  ...
  return (
    <div className="p-30 bg-gray-100 min-h-screen">
        ...
        <table className="w-full bg-white shadow-md rounded-lg">
          ...
          <tbody>
            {filteredSales.map((sale) => (
              <tr key={sale.id}>
                <td className="px-4 py-2">{sale.propertyName}</td>
      ...
      >
        <ViewTransactions setIsView={setIsView} selectedTransaction={selectedTransaction} />
      </Modal>
    </div>
  );
};
