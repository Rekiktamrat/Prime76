...
const RentalTransactions = () => {
  ...
  const { rents, loading, error } = useSelector((state) => state.transaction);

  const [searchQuery, setSearchQuery] = useState("");
  const [priceFilter, setPriceFilter] = useState("");

  useEffect(() => {
    dispatch(getTransactionsbyregion());
  }, [dispatch]);
  ...
  const filteredTransactions = rents?.filter((transaction) => {
    return (
          ...
          <tbody>
            {filteredTransactions?.map((transaction) => (
              <tr key={transaction.id}>
                <td className="px-4 py-2">{transaction.propertyName}</td>
                <td className="px-4 py-2">{transaction.renterName}</td>
