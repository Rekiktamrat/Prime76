...
import { deleteRegion } from "../../../store/address/region/regionSlice";

const DeleteRegion = ({ setIsDelete, selectedRegion }) => {
  const dispatch = useDispatch();

  const handleDelete = () => {
    // Dispatch the action to delete the region
    dispatch(deleteRegion(selectedRegion.id))
      .then(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-gray-600 dark:text-gray-300">
        ...
        <span className="font-medium">{selectedRegion.name}</span>? This action
        cannot be undone.
      </p>
