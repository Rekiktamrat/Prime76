...
const Location = () => {
  ...
  const groupedLocations = locations?.reduce((acc, location) => {
    const countryId = location.country?.id || "uncategorized";
    const regionId = location.region?.id || "uncategorized";
    const subregionId = location.subregion?.id || "uncategorized";
      ...
      {locations?.length > 0 ? (
        <div className="space-y-6">
          {Object.values(groupedLocations).map((group) => (
            <div key={`${group.country.id}-${group.region.id}-${group.subregion.id}`} className="border-b pb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                {group.country.country_name} - {group.region.region_name} - {group.subregion.subregion_name}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {group.locations.map((location) => (
                  <div key={location.id} className="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-lg transition-shadow duration-200 p-4 relative">
                    <h3 className="text-md font-medium text-gray-800 dark:text-white truncate">{location.location}</h3>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">Click the "Add Location" button to create one.</p>
        </div>
      )}

      {/* Modals for adding/editing/deleting locations */}
      <Modal isOpen={isAdd} onRequestClose={() => setIsAdd(false)} style={customModalStyles} contentLabel="Add Location">
        <AddLocation setIsAdd={setIsAdd} />
      </Modal>

      <Modal isOpen={isEdit} onRequestClose={() => setIsEdit(false)} style={customModalStyles} contentLabel="Edit Location">
        <EditLocation setIsEdit={setIsEdit} selectedLocation={modifyLocation} />
      </Modal>

      <Modal isOpen={isDelete} onRequestClose={() => setIsDelete(false)} style={customModalStyles} contentLabel="Delete Location">
        <DeleteLocation setIsDelete={setIsDelete} selectedLocation={modifyLocation} />
      </Modal>
