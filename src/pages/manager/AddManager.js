...
const AddManager = ({ setIsAdd }) => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "region" ? parseInt(value, 10) : value, // Ensure region is stored as an integer
    }));
  };

  const filteredSubRegions = subRegions.filter(
    (subregion) => subregion.region_id === formData.region
  );
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      sub_region_id: formData.subregion,
    };
