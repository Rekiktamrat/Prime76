...
const addSubRegion = async (data) => {
  ...
  const response = await axios.post(`${base_url}/subregions`, data);
  return response.data;
};

const getAllSubRegions = async () => {
  const response = await axios.get(`${base_url}/subregions`);
  return response.data;
};
...
const deleteSubRegion = async (id) => {
  ...
  const response = await axios.delete(`${base_url}/subregions${id}`);
  return response.data;
};
